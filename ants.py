...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while curr_place is not hive and distance <= self.max_range:
            if bees_place:
                if distance >= self.min_range:
                    return random_or_none(bees_place)
            curr_place = curr_place.entrance
            bees_place = curr_place.bees
            distance += 1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            for bees_place in self.place.bees[:]:
                bees_place.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        for single_bee in bee:
            single_bee.armor = 0
            self.place.remove_insect(single_bee)
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt will slow down the bees from attacking."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can survive on water and do the action."""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.first_deploy:
            self.true_queen = True
            QueenAnt.first_deploy = True
        # END Problem 13
            ...
            while curr_place:
                if curr_place.ant:
                    if curr_place.ant.is_container:
                        if curr_place.ant.contained_ant:
                            if curr_place.ant.contained_ant not in self.doubled_ants:
                                curr_place.ant.contained_ant.damage *= 2
                                self.doubled_ants += [curr_place.ant.contained_ant]
                        if curr_place.ant is not BodyguardAnt and curr_place.ant not in self.doubled_ants:
                            curr_place.ant.damage *= 2
                            self.doubled_ants += [curr_place.ant]
                    elif curr_place.ant not in self.doubled_ants:
                        curr_place.ant.damage *= 2
                        self.doubled_ants += [curr_place.ant]
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    def helper_colony(colony):
        if colony.time % 2 == 0:
            action(colony)
    return helper_colony
    # END Problem EC
